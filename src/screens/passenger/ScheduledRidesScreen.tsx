...
export function ScheduledRidesScreen({ navigation }: Props) {
  ...
  async function handleActivate(ride: RideRecord) {
    ...
    if (activated) {
      navigation.navigate('FindingDriver', { ride: activated });
    } else {
      Alert.alert('Ops', 'Não foi possível solicitar a corrida. Tente novamente.');
    }
  }
    ...
    <SafeAreaView style={styles.container}>
      ...
      {loading && rides.length === 0 ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => <ScheduledCard ride={item} onActivate={handleActivate} onCancel={handleCancel} />}
        />
      )}
    </SafeAreaView>
  );
    ...
    <View style={styles.card}>
      <View style={styles.cardTop}>
        ...
        {isDue ? (
          ...
          <View style={styles.schBadge}><Text style={styles.schText}>Agendada</Text></View>
        )}
      </View>
      ...
      <View style={styles.cardActions}>
        ...
        <TouchableOpacity style={styles.activateBtn} onPress={() => onActivate(ride)}>
          <Text style={styles.activateBtnText}>Solicitar agora</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  schBadge: { backgroundColor: 'rgba(201,168,76,0.15)', borderRadius: 8, paddingHorizontal: 8, paddingVertical: 3 },
  schText: { color: Colors.accent, fontSize: 11, fontWeight: '700' },
  dueBadge: { backgroundColor: 'rgba(34,197,94,0.12)', borderRadius: 8, paddingHorizontal: 8, paddingVertical: 3 },
  dueText: { color: Colors.success, fontSize: 11, fontWeight: '700' },
