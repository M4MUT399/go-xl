...
Deno.serve(async (req) => {
  ...
  // ── Páginas de retorno do Checkout (fallback — ver comentário no POST) ──────
  if (req.method === 'GET') {
  ...
  try {
    ...
    // success_url/cancel_url apontam para o deep link do app (scheme "goxl"),
    // não para esta própria função. O app abre o Checkout via
    // WebBrowser.openAuthSessionAsync, que FECHA o navegador sozinho assim que
    // detecta esse redirect — o usuário nunca chega a ver uma página web
    // intermediária (nem a de erro 401 antiga, nem a de sucesso em HTML), só
    // a tela nativa de confirmação do próprio app. A página HTML abaixo
    // continua existindo como fallback (ex.: se o link for aberto fora do
    // fluxo normal do app).
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: 'goxl://card-return?status=success',
      cancel_url:  'goxl://card-return?status=cancel',
    });
