...
Deno.serve(async (req) => {
  ...
  // ── Páginas de retorno do Account Link (fallback — ver comentário no POST) ──
  if (req.method === 'GET') {
  ...
  try {
    ...
    // refresh_url/return_url apontam para o deep link do app (scheme "goxl"),
    // não para esta própria função — mesmo padrão do setup-card. O app abre
    // via WebBrowser.openAuthSessionAsync, que fecha o navegador sozinho ao
    // detectar esse redirect, sem o motorista ver nenhuma página web.
    const link = await stripe.accountLinks.create({
      account: accountId,
      refresh_url: 'goxl://payout-return?status=refresh',
      return_url:  'goxl://payout-return?status=return',
